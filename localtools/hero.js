...
(function (global) {
  ...
  /** Extra viewBox width so particles can dissolve past the mark without hard clip */
  const FADE_PAD = 52;
  const VIEW_W = VIEW.w + FADE_PAD;
  /** Right edge of the W mark (before fade pad) */
  const MARK_RIGHT = VIEW.x + VIEW.w;
  /** Soft dissolve starts before the mark edge, finishes in the pad zone */
  const FADE_START = MARK_RIGHT - 44;
  const FADE_END = MARK_RIGHT + FADE_PAD - 4;
  const EXIT_X = FADE_END;
  const TRAIL_CUT = 337;
  const REPEL_R = 72;

  /** Slow molasses (mockup V3) — fade is absolute (FADE_START→FADE_END), not a hard clip */
  const STREAM = {
    speed: 6,
    speedJitter: 0.05,
    yJitter: 0,
  ...
  function buildSvg(tiles) {
    ...
    return (
      ...
      " " +
      VIEW.h +
      '" xmlns="http://www.w3.org/2000/svg" aria-label="Warp" overflow="visible">' +
      '<g class="core-layer">' +
      cores +
      "</g>" +
  ...
  /** Smoothstep 0→1 for natural ease (no linear hard cut). */
  ...
  function paintParticle(p) {
    ...
    let op = 1;
    // Fade in as copies leave the trail origin
    const fadeInEnd = TRAIL_CUT + STREAM.fadeInPx;
    if (p.x < fadeInEnd) {
      op *= smoothstep((p.x - TRAIL_CUT) / Math.max(1, STREAM.fadeInPx));
    }
    // Fade out slowly past FADE_START → fully gone by FADE_END (no hard clip)
    if (p.x >= FADE_START) {
      const t = (p.x - FADE_START) / Math.max(1, FADE_END - FADE_START);
      // ease-in on the fade (starts gentle, finishes soft)
  ...
  function tick(now) {
    ...
    for (let i = 0; i < engine.particles.length; i++) {
      ...
      p.age += dt;
      p.x += p.speed * dt;
      // Respawn only after fully faded (past FADE_END) — never mid-clip
      if (p.x >= EXIT_X) {
